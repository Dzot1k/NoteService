...
object CommentsService : CrudService<Comments> {
    ...
    fun getByIdNote(idNote: Int): MutableList<Comments> {
        val commentsForGet = mutableListOf<Comments>()
        for (note in notes) {
            if (idNote == note.id && !note.isDelete) {
                for (comment in note.comments) {
                    if (!comment.isDelete) {
                        commentsForGet += comment
                    }
        ...
        throw NoteNotFoundException("not note with id: $idNote")
    }
